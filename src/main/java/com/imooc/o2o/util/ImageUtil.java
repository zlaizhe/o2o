...
public class ImageUtil {
    private static String classpathUrl = Thread.currentThread().getContextClassLoader().getResource("").getPath();
    ...
    static {
        try {
            basePath = URLDecoder.decode(classpathUrl, "UTF-8");
...
//        Thumbnails.of(new File("C:/USER/桌面/xiaohuangren.jpg"))
//                .size(200, 200).watermark(Positions.BOTTOM_RIGHT, ImageIO.read(new File(basePath + "/watermark.jpg")), 0.25f)
//                .outputQuality(0.8f).toFile("C:/USER/桌面/xiaohuangrennew.jpg");
//    }
}
